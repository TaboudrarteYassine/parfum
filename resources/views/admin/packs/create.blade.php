...
@section('header', 'Créer un Pack')
@section('content')
<div class="bg-card rounded-lg border border-border shadow-sm p-6">
    <form action="{{ route('admin.packs.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" class="w-full bg-dark border border-border rounded px-4 py-2 text-white focus:outline-none focus:border-gray-500" required>
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-400 mb-1">Prix (€)</label>
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full bg-dark border border-border rounded px-4 py-2 text-white focus:outline-none focus:border-gray-500" required>
            </div>
            <div class="md:col-span-2">
                <label class="block text-sm font-medium text-gray-400 mb-1">Description</label>
                <textarea name="description" rows="3" class="w-full bg-dark border border-border rounded px-4 py-2 text-white focus:outline-none focus:border-gray-500" required>{{ old('description') }}</textarea>
            </div>
            <div>
        ...
        <div class="flex gap-4">
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded">Créer le Pack</button>
